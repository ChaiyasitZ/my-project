...
\begin{mycustomitem2}
    \item มีคำสั่ง \textbf{interface}, มี IP Address, มีคำสั่ง OSPF, มี Network Statement
    \item มี Router-ID, ไม่มี Markdown formatting, ไม่มีคำอธิบาย, มี Wildcard Mask ถูกต้อง
    \item ไม่มี \textbf{configure terminal}, มี IP ถูกต้อง (10.0.0.1)
\end{mycustomitem2}

\hspace*{1.3em}
\textbf{เกณฑ์การให้คะแนน NETCONF} (10 เกณฑ์ เกณฑ์ละ 10\%):
\begin{mycustomitem2}
    \item มีโครงสร้าง XML (\textbf{<} และ \textbf{>}), มี \textbf{xmlns} namespace, ไม่มี Markdown
    \item ไม่มีคำอธิบาย, มี IP Address ถูกต้อง
    \item มี Root Element ตรงตามประเภทอุปกรณ์ (IOS: \textbf{interfaces}, IOS-XE: \textbf{native}, NX-OS: \textbf{System})
    \item มี Namespace ถูกต้องตามอุปกรณ์, มี Element เฉพาะอุปกรณ์ (GigabitEthernet, PhysIf-list ฯลฯ)
    \item มี Element \textbf{enabled/shutdown/adminSt}, มี Subnet Mask ถูกต้อง
\end{mycustomitem2}
...
\hspace*{1.3em}
จากตารางที่ \ref{tab:benchmark-cli} พบว่า \textbf{codegemma:7b} ได้คะแนนสูงสุดที่ 90\% ในทุกระบบปฏิบัติการ เนื่องจากสามารถสร้างคำสั่ง \textbf{interface}, \textbf{ip address} พร้อม Subnet Mask ที่ถูกต้อง, \textbf{no shutdown} และคำสั่ง OSPF ได้ครบถ้วน อย่างไรก็ตาม \textbf{codegemma:7b} ใช้เวลาตอบสนองนานที่สุด (ค่าเฉลี่ย 10,145 ms) และใช้ RAM สูงสุด (ค่าเฉลี่ย 17,120 MB)

\hspace*{1.3em}
\textbf{codellama:7b} ได้คะแนน 80\% โดยมีความเร็วในการสร้าง Token สูงสุดที่ 11.8 tok/s แต่ขาดความถูกต้องในบางเกณฑ์ เช่น การใส่ Wildcard Mask ที่ไม่ถูกต้อง ส่วน \textbf{qwen2.5-coder:7b} แม้จะตอบสนองเร็วที่สุด (ค่าเฉลี่ย 3,705 ms) และใช้ RAM ต่ำสุด (ค่าเฉลี่ย 15,040 MB) แต่ได้คะแนนเพียง 70\% เนื่องจากไม่สร้างคำสั่ง \textbf{interface} แยก โดยสร้างเฉพาะคำสั่ง OSPF เท่านั้น
...
\hspace*{1.3em}
\textbf{qwen2.5-coder:7b} ได้คะแนนสูงสุดสำหรับ IOS ที่ 100\% โดยสามารถสร้าง IETF-compliant XML ได้อย่างสมบูรณ์ พร้อม \textbf{xmlns} ที่ถูกต้อง (\textbf{urn:ietf:params:xml:ns:yang:ietf-interfaces}) และมี Element ครบถ้วนทั้ง \textbf{enabled}, \textbf{netmask} และ \textbf{type}

\hspace*{1.3em}
\textbf{codegemma:7b} ได้คะแนนสูงสุดสำหรับ NX-OS ที่ 100\% ทั้ง 10 รอบ โดยสร้าง Cisco NX-OS Device YANG XML ได้สมบูรณ์ที่สุด พร้อม \textbf{xmlns} (\textbf{http://cisco.com/ns/yang/cisco-nx-os-device}) และโครงสร้าง \textbf{intf-items}, \textbf{PhysIf-list}, \textbf{adminSt} ที่ถูกต้อง แต่ใช้เวลาตอบสนองนานที่สุดที่ 37,178 ms

\hspace*{1.3em}
\textbf{codellama:7b} ได้คะแนน NX-OS ต่ำสุดที่ 89\% (ช่วง 80--90\%) เนื่องจากบางรอบขาดความถูกต้องของ Namespace หรือ Root Element
...
\hspace*{1.3em}
\textbf{(1) ด้านคุณภาพ:} \textbf{codegemma:7b} ได้คะแนนเฉลี่ยสูงสุดที่ 91.8\% โดยเป็นโมเดลเดียวที่ได้คะแนน CLI ถึง 90\% ในทุกระบบปฏิบัติการ และได้คะแนน NETCONF NX-OS เต็ม 100\% ทั้ง 10 รอบ รองลงมาคือ \textbf{codellama:7b} ที่ 84.8\% และ \textbf{qwen2.5-coder:7b} ที่ 81.7\%

\hspace*{1.3em}
\textbf{(2) ด้านความเร็ว:} \textbf{qwen2.5-coder:7b} ตอบสนองเร็วที่สุดสำหรับ CLI (เฉลี่ย 3,705 ms) เร็วกว่า \textbf{codellama:7b} (4,376 ms) ประมาณ 18\% และเร็วกว่า \textbf{codegemma:7b} (10,145 ms) ประมาณ 2.7 เท่า สำหรับอัตราการสร้าง Token \textbf{codellama:7b} เร็วที่สุดที่ 11.5 tok/s

\hspace*{1.3em}
\textbf{(3) ด้านทรัพยากร:} \textbf{qwen2.5-coder:7b} ใช้ RAM ต่ำสุดเฉลี่ย 15,065 MB ขณะที่ \textbf{codegemma:7b} ใช้สูงสุดเฉลี่ย 17,128 MB (มากกว่าประมาณ 2 GB)

\hspace*{1.3em}
\textbf{(4) ด้านความสม่ำเสมอ:} ทุกโมเดลมีความสม่ำเสมอสูง โดย \textbf{qwen2.5-coder:7b} และ \textbf{codegemma:7b} ได้คะแนนคงที่ในแต่ละรอบ (Standard Deviation ของคะแนน = 0) ยกเว้น \textbf{codellama:7b} NETCONF NX-OS ที่มีช่วงคะแนน 80--90\% และ \textbf{codegemma:7b} NETCONF IOS ที่มีช่วง 90--100\%
...
\hspace*{1.3em}
\textbf{โมเดลที่แนะนำ: \textbf{codegemma:7b}} -- มีคะแนนคุณภาพสูงสุดเฉลี่ย 91.8\% ครอบคลุมทั้ง CLI (90\%) และ NETCONF (93.7\%) แม้จะใช้เวลาและทรัพยากรมากกว่าโมเดลอื่น แต่ในบริบทการสร้างคอนฟิกเครือข่ายที่ต้องการความถูกต้องสูง ความแม่นยำมีความสำคัญมากกว่าความเร็ว

\hspace*{1.3em}
\textbf{ทางเลือกสำหรับทรัพยากรจำกัด: \textbf{qwen2.5-coder:7b}} -- ใช้ RAM ต่ำสุดและตอบสนองเร็วที่สุด เหมาะสำหรับเครื่องที่มี RAM จำกัด โดยเฉพาะงาน NETCONF IOS ที่ได้คะแนน 100\%

\hspace*{1.3em}
\textbf{\textbf{codellama:7b}} -- มีความสมดุลระหว่างคุณภาพและความเร็ว เหมาะสำหรับงาน CLI ที่ต้องการความเร็วในการสร้าง Token สูง (11.8 tok/s)
