...
\begin{mycustomenum2}
    \item \textbf{Transport Layer:} ใช้ SSH (Secure Shell) เป็นช่องทางการสื่อสาร โดยเชื่อมต่อผ่าน SSH Subsystem ชื่อ \texttt{netconf} ที่พอร์ต 830 (ค่าเริ่มต้นมาตรฐาน)
    \item \textbf{Messages Layer:} ใช้ XML-based RPC (Remote Procedure Call) ในการส่งคำสั่งและรับผลลัพธ์ โดยข้อความทุกข้อความสิ้นสุดด้วย Delimiter \texttt{]]>]]>} เพื่อระบุจุดสิ้นสุดของข้อความ
    \item \textbf{Operations Layer:} กำหนดชุดคำสั่ง (Operations) ที่สามารถดำเนินการได้ เช่น \texttt{get-config}, \texttt{edit-config}, \texttt{lock}, \texttt{unlock}, \texttt{commit}, \texttt{validate} และ \texttt{close-session}
...
\hline
Cisco NX-OS (Nexus) & \texttt{http://cisco.com/ns/yang/cisco-nx-os-device} & \texttt{System} \\
\hline
Cisco IOS-XE & \texttt{http://cisco.com/ns/yang/Cisco-IOS-XE-native} & \texttt{native} \\
\hline
\end{tabularx}
\end{table}
...
\begin{mycustomenum2}
    ...
    \item \textbf{list:} โหนดที่มีข้อมูลหลายรายการ โดยแต่ละรายการใช้ \texttt{key} ในการระบุตัวตน
...
\hspace*{1.3em}
ในระบบนี้ผู้ใช้สามารถอัปโหลด YANG Model เองได้ โดยระบบจะ Parse ข้อมูลจาก YANG Content อัตโนมัติ ดึงค่า \texttt{module}, \texttt{namespace}, \texttt{prefix} และ \texttt{revision} มาจัดเก็บในฐานข้อมูล นอกจากนี้แต่ละ YANG Model สามารถมี XML Templates เพื่อใช้เป็น Template สำหรับ LLM ในการสร้าง Configuration ได้
...
\begin{mycustomenum2}
    \item \textbf{Key Exchange:} \texttt{diffie-hellman-group14-sha256}, \texttt{ecdh-sha2-nistp256}
    \item \textbf{Cipher:} \texttt{aes256-ctr}, \texttt{aes128-ctr}
    \item \textbf{Connection Timeout:} 30 วินาที
\end{mycustomenum2}

\subsection{ขั้นตอนที่ 2: NETCONF Subsystem}
\hspace*{1.3em}
เมื่อ SSH Connection สำเร็จ ระบบจะเปิด NETCONF Subsystem ผ่านคำสั่ง \texttt{conn.subsys('netconf')} เพื่อเข้าสู่โหมดการสื่อสาร NETCONF โดยเฉพาะ ซึ่งจะได้ Stream สำหรับรับ-ส่งข้อความ XML
...
\hspace*{1.3em}
\textbf{Server Hello:} อุปกรณ์เครือข่ายจะส่ง Hello Message มาก่อน ระบบจะ Parse เพื่อดึง Capabilities ที่อุปกรณ์รองรับ เช่น \texttt{base:1.0}, \texttt{writable-running}, \texttt{candidate} เป็นต้น
...
\hspace*{1.3em}
เมื่อทั้งสองฝ่ายแลกเปลี่ยน Hello Message สำเร็จ NETCONF Session จะถูกสร้างขึ้นและพร้อมสำหรับการดำเนินการ RPC ระบบจะจัดเก็บ Session ไว้ใน \texttt{connections Map} พร้อมข้อมูล Capabilities, Stream และ Timestamp เพื่อใช้ในการดำเนินการต่อไป
...
\hspace*{1.3em}
ระบบใช้ NETCONF RPC Operations หลายชนิดในการจัดการคอนฟิกอุปกรณ์เครือข่าย ทุก RPC Message จะถูกห่อด้วย \texttt{<rpc>} Envelope พร้อมระบุ \texttt{message-id} ที่ไม่ซ้ำกัน และสิ้นสุดด้วย Delimiter \texttt{]]>]]>} โดยรูปแบบทั่วไปของ RPC Message มีดังนี้
...
\hspace*{1.3em}
ใช้สำหรับแก้ไขค่า Configuration ของอุปกรณ์ สามารถเลือก Target Datastore เป็น \texttt{running} (แก้ไขทันที) หรือ \texttt{candidate} (แก้ไขเป็นร่างก่อน) และกำหนด Default Operation เป็น \texttt{merge} (รวมกับค่าเดิม) หรือ \texttt{replace} (แทนที่ค่าเดิม)
...
\hline
อ่านคอนฟิก & \texttt{get-config} & 180 วินาที & อ่าน Running Configuration \\
\hline
อ่านข้อมูล & \texttt{get} & 120 วินาที & อ่าน Operational Data \\
\hline
แก้ไขคอนฟิก & \texttt{edit-config} & 90 วินาที & แก้ไข Running/Candidate \\
\hline
ยืนยัน & \texttt{commit} & 120 วินาที & ยืนยัน Candidate $\rightarrow$ Running \\
\hline
ตรวจสอบ & \texttt{validate} & 60 วินาที & ตรวจสอบ Candidate \\
\hline
ยกเลิก & \texttt{discard-changes} & 15 วินาที & ยกเลิก Candidate \\
\hline
ล็อก/ปลดล็อก & \texttt{lock/unlock} & 30 วินาที & ล็อก Datastore \\
\hline
ปิด Session & \texttt{close-session} & 10 วินาที & ปิด NETCONF Session \\
\hline
\end{tabularx}
\end{table}
...
\begin{mycustomenum2}
    ...
    \item \textbf{Edit-Config:} ส่ง Configuration XML ไปยัง Candidate Datastore ด้วย \texttt{edit-config}
    \item \textbf{Validate (ถ้ารองรับ):} ตรวจสอบความถูกต้องของ Candidate ด้วย \texttt{validate}
    \item \textbf{Commit:} ยืนยันการเปลี่ยนแปลงจาก Candidate ไปยัง Running ด้วย \texttt{commit}
...
\hspace*{1.3em}
สำหรับอุปกรณ์ที่ไม่รองรับ Candidate Datastore ระบบจะใช้ \texttt{writable-running} เป็นทางเลือก โดยการเปลี่ยนแปลงจะมีผลทันทีเมื่อ edit-config สำเร็จ ขั้นตอนมีดังนี้
...
\hspace*{1.3em}
ระบบใช้ Subtree Filter ใน \texttt{<get>} RPC เพื่ออ่านข้อมูลเฉพาะส่วนที่ต้องการจากอุปกรณ์ โดยไม่ต้องอ่านข้อมูลทั้งหมด ซึ่งช่วยลดปริมาณข้อมูลที่รับ-ส่งและเพิ่มประสิทธิภาพ ระบบรองรับ Filter หลายประเภทตามชนิดของอุปกรณ์ ดังนี้

\subsection{Subtree Filter สำหรับ NX-OS (Nexus)}
\hspace*{1.3em}
YANG Model ของ Cisco NX-OS ใช้ Root Element \texttt{System} ภายใต้ Namespace \texttt{cisco-nx-os-device} ตัวอย่าง Filter ที่ใช้ในระบบมีดังนี้
...
\hline
System Info & \texttt{System/name} & ชื่ออุปกรณ์ (Hostname) \\
\hline
Interfaces & \texttt{System/intf-items} & อินเทอร์เฟซทั้งหมด (Physical, SVI, Port-channel) \\
\hline
VLANs & \texttt{System/bd-items} & Bridge Domain / VLAN \\
\hline
OSPF & \texttt{System/ospf-items} & Routing Protocol OSPF \\
\hline
BGP & \texttt{System/bgp-items} & Routing Protocol BGP \\
\hline
Features & \texttt{System/fm-items} & Feature Manager (เปิด/ปิดฟีเจอร์) \\
\hline
\end{tabularx}
\end{table}
...
\hspace*{1.3em}
YANG Model ของ Cisco IOS-XE ใช้ Root Element \texttt{native} ภายใต้ Namespace \texttt{Cisco-IOS-XE-native} ตัวอย่าง Filter ที่ใช้ในระบบมีดังนี้
...
\textbf{ประเภทข้อมูล} & \textbf{YANG Path} & \textbf{คำอธิบาย} \\
\hline
Hostname & \texttt{native/hostname} & ชื่ออุปกรณ์ \\
\hline
Interfaces & \texttt{native/interface} & อินเทอร์เฟซ GigabitEthernet/Loopback \\
\hline
VLANs & \texttt{native/vlan} & VLAN Configuration \\
\hline
OSPF & \texttt{native/router/ospf} & Routing Protocol OSPF \\
\hline
BGP & \texttt{native/router/bgp} & Routing Protocol BGP \\
\hline
\end{tabularx}
\end{table}
...
\begin{mycustomenum2}
    \item \textbf{module name:} ชื่อโมดูล YANG เช่น \texttt{cisco-nx-os-device}
    \item \textbf{namespace:} URI ของโมดูล เช่น \texttt{http://cisco.com/ns/yang/cisco-nx-os-device}
    \item \textbf{prefix:} ชื่อย่อสำหรับอ้างอิง เช่น \texttt{nxos}
    \item \textbf{revision:} เวอร์ชันของ YANG Model เช่น \texttt{2024-01-01}
\end{mycustomenum2}
...
\hspace*{1.3em}
เมื่ออุปกรณ์พบข้อผิดพลาดในการดำเนินการ NETCONF จะตอบกลับด้วย \texttt{<rpc-error>} ระบบจะ Parse ข้อมูลใน Error Message เพื่อแสดงรายละเอียดให้ผู้ใช้ทราบ โดย Error Message ประกอบด้วย

\begin{mycustomenum2}
    \item \textbf{error-tag:} ประเภทของข้อผิดพลาด เช่น \texttt{invalid-value}, \texttt{data-missing}, \texttt{operation-failed}
    \item \textbf{error-message:} คำอธิบายข้อผิดพลาดเป็นข้อความภาษาอังกฤษ
    \item \textbf{error-path:} ตำแหน่งใน YANG Model ที่เกิดข้อผิดพลาด เช่น \texttt{/System/intf-items}
\end{mycustomenum2}
