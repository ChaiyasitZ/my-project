...
% ภาคผนวก จ.
%*************************************
\appendixChapterTitle{ฉ}{ซอร์สโค้ดหลักของระบบ}
\vspace*{-0.8in}
...
export const authenticateToken = async (req, res, next) => {
  try {
    /* ... omitted authentication logic for brevity ... */
    req.user = { id: '...' };
    next();
  } catch (error) {
...
\begin{lstlisting}[language=JavaScript, caption={llmService.js -- Chat Completion และ Ollama Relay}]
  async _callChatCompletion(messages, params = {}, userId = null) {
    /* ... omitted local and openrouter API split ... */
  }

  async _chatViaOllama(userId, messages, params = {}) {
    /* ... omitted agent relay mapping ... */
    return { pending: true };
  }
\end{lstlisting}
...
\begin{lstlisting}[language=JavaScript, caption={llmService.js -- generateConfiguration}]
  async generateConfiguration(
    ...
    try {
      /* ... omitted cache, rate limiting, template building, and API retry blocks ... */
      const rawConfig = '...';
      const deployConfig = '...';
      const validation = { score: 100 };
...
\hspace*{1.3em}
บริการสำหรับเชื่อมต่อ SSH ไปยังอุปกรณ์เครือข่าย รองรับ Session Reuse, การ Deploy Configuration ผ่าน Shell
...
\begin{lstlisting}[language=JavaScript, caption={netconfService.js -- connect}]
  async connect(deviceConfig) {
    ...
    return new Promise((resolve, reject) => {
      /* ... omitted SSH subystem framing and NETCONF hello exchange for brevity ... */
      resolve({ success: true, message: 'NETCONF session established' });
    });
  }
\end{lstlisting}
...
\begin{lstlisting}[language=JavaScript, caption={netconfService.js -- sendRpc และ editConfig}]
  async sendRpc(deviceId, rpcContent,
    ...
    /* ... omitted XML framing and data chunking logic for brevity ... */
    return { success: true, messageId: 'msg-1', response: '<ok/>' };
  }
...
export class ConsoleService {
  ...
  async sendInitialConfig(deviceId, configCommands) {
    ...
    /* ... omitted command loop for brevity ... */
...
class AgentRelay {
  ...
  async sendToAgent(userId, event, data,
    ...
    /* ... omitted command polling logic for brevity ... */
...
router.get('/', async (req, res) => {
  try {
    ...
    const devices = await Device.aggregate([ /* ... omitted aggregation pipeline for brevity ... */ ]);
...
router.put('/:id', async (req, res) => {
  try {
    /* ... omitted PUT device update logic for brevity ... */
...
% ผู้จัดทำคนที่ 1
\begin{center}
\fbox{\includegraphics[width=2in,height=2in,keepaspectratio]{Image/profile.jpg}}
\end{center}
