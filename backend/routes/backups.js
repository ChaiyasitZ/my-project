...
// POST /api/backups/post-deployment - Create backup after successful deployment
router.post('/post-deployment', async (req, res) => {
  try {
    const { error, value } = createBackupSchema.validate(req.body);
    ...
    console.log(`📦 Creating post-deployment backup for ${device.name} (${device.ip_address})`);
    
    try {
      // Use existing session to backup (after deployment)
      const backupResult = await sshService.optimizedBackup(device, 'running-config');
      
      if (!backupResult.success) {
        throw new Error('Failed to create post-deployment backup');
      }
      ...
      // Save backup to database with scheduled type
      const backup = new ConfigurationBackup({
        device_id,
        backup_name,
        description: description || 'Post-deployment automatic backup',
        running_config: backupResult.runningConfig,
        startup_config: backupResult.startupConfig,
        backup_type: 'scheduled', // Mark as scheduled/automatic
        config_type: 'running-config',
        file_size: backupResult.runningConfigSize + (backupResult.startupConfigSize || 0),
        config_hash: configHash,
        created_by: created_by || 'system',
        tags: tags || ['post-deployment', 'auto-backup']
      });
      ...
      res.status(201).json({
        success: true,
        message: 'Post-deployment backup created successfully',
        backup: {
          id: backup._id,
          backup_name: backup.backup_name,
          device_name: device.name,
      ...
      console.error('Post-deployment backup error:', backupError.message);
      
      res.status(500).json({
        success: false,
        message: `Post-deployment backup failed: ${backupError.message}`,
        error: backupError.message,
        device: {
          name: device.name,
          ip_address: device.ip_address
    ...
    console.error('Error creating post-deployment backup:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to create post-deployment backup'
    });
  }
});
