...
router.post('/generate', async (req, res) => {
  try {
    ...
    // Private research logging only (see modelComparisonService.js): dispatched
    // AFTER the OpenRouter response is already finalized (not concurrently),
    // so Ollama's real background execution doesn't overlap the UX delay
    // window above. Fire-and-forget — never awaited, never shown in the UI,
    // never affects what was just generated or returned.
...
router.post('/netconf/apply', async (req, res) => {
  try {
    ...
    console.log(`🌐 NETCONF Apply: Deploying to ${device.name} (${device.ip_address}) via SSH (dual-mode)`);
    ...
    try {
      ...
      if (isCliConfig) {
        // Dual-mode: deployment_config is already CLI commands, deploy via SSH
        ...
        // Deploy CLI commands via SSH using the same mechanism as /apply
        const sshResult = await deployViaAgent(req.userId, device, configToApply);
        const deploymentTime = Date.now() - deploymentStart;
      ...
      // Fallback: deployment_config is still XML (legacy or conversion failed)
      // Use the original NETCONF edit-config path
