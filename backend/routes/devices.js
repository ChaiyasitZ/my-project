...
// POST /api/devices/:id/ssh/exec - Execute a show command via SSH (alternative to NETCONF)
router.post('/:id/ssh/exec', async (req, res) => {
  try {
    ...
    // Route through agent relay
    const agentOnline = await agentRelay.isAgentOnline(req.userId);
    
    if (agentOnline) {
      const result = await agentRelay.sendToAgent(req.userId, 'agent:ssh:exec-show', {
        deviceId: id,
        host: device.ip_address,
        port: device.ssh_port || 22,
        username: device.username,
    ...
    // Direct SSH fallback (desktop/local mode)
    try {
      const result = await sshService.executeCommandWithEnable(device, command);
      const executionTime = Date.now() - startTime;
      ...
      throw new Error(`SSH command failed: ${sshError.message}`);
    }
    ...
    res.status(500).json({
      success: false,
      message: error.message || 'Failed to execute SSH command'
    });
  }
});
