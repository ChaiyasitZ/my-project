...
function ConsoleConfiguration() {
  ...
  const [isMockMode, setIsMockMode] = useState(false); // True when using a mock port
  ...
  const isMockPort = (port) => availablePorts.find(p => p.path === port)?.isMock;
  ...
  const handleTestConnection = async () => {
    ...
    // Mock port — simulate test success
  ...
  const handleApplyConfiguration = async () => {
    ...
    // Mock mode — simulate configuration apply
    if (isMockMode) {
      setIsLoading(true);
      await new Promise(r => setTimeout(r, 1500));
      const commands = configMode === 'custom' 
        ? customConfig.split('\n').filter(l => l.trim())
