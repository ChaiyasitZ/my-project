...
function BackupManagement() {
  ...
  const handleRestoreBackup = async (e) => {
    ...
    try {
      ...
      alert('Configuration restored successfully!\n\n' + 
        `Device: ${response.data.restore_details.device_name}\n` +
        `Backup: ${response.data.restore_details.backup_name}\n` +
  ...
  const handleSetRestorePoint = async (backup) => {
    try {
      ...
      alert(`Backup "${backup.backup_name}" marked as restore point!`);
      fetchData();
    } catch (error) {
      console.error('Error setting restore point:', error);
      alert('Error setting restore point: ' + (error.response?.data?.message || error.message));
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Configuration Backups</h1>
