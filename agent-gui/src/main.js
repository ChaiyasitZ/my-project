...
class HttpPollingClient extends EventEmitter {
  constructor({ serverUrl, agentToken, agentName, version, handlers }) {
    ...
    this.cachedCpuInfo = null; // Cache CPU usage/model for heartbeat (needs a 200ms sampling window, so refreshed periodically too)
  }
  ...
  async connect() {
    ...
    // Refresh cached CPU usage every 5s (matches heartbeat cadence — the
    // 200ms sampling window is cheap, just kept off the heartbeat's own
    // critical path for consistency with how GPU info is handled)
    ...
    try {
      ...
      switch (event) {
        ...
        case 'agent:ssh:get-interfaces': {
          // All-in-one: connect → run interface-list command → disconnect.
          // Used to verify LLM-generated interface names against the real
          // device before deploying.
          const ifData = data;
          try {
            await this.handlers.ssh.connect({ deviceId: ifData.deviceId, host: ifData.host, port: ifData.port || 22, username: ifData.username, password: ifData.password });
            result = await this.handlers.ssh.executeCommand(ifData.deviceId, ifData.command || 'show ip interface brief');
            this.handlers.ssh.disconnect(ifData.deviceId);
          } catch (ifErr) {
            this.handlers.ssh.disconnect(ifData.deviceId);
            throw ifErr;
          }
  ...
  async disconnect() {
    ...
    if (this.cpuRefreshInterval) { clearInterval(this.cpuRefreshInterval); this.cpuRefreshInterval = null; }
